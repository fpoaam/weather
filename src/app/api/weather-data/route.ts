...
async function fetchFromAzure(containerName: string, connectionIndex: 0 | 1 | 2): Promise<WeatherDataPoint[]> {
  ...
  const data: WeatherDataPoint[] = parsed.data || [];
  console.log(`✅ [weather-data] Loaded ${data.length} data points from ${containerName}`);
  return data;
}

async function getData(containerName: string, connectionIndex: 0 | 1 | 2): Promise<WeatherDataPoint[]> {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ── 2. Access check (admins bypass, regular users need ContainerAccess) ────
    ...
    if (!user.isAdmin) {
      const access = await prisma.containerAccess.findUnique({
        where: {
    ...
    if (startDate || endDate) {
      ...
      const filtered = allData.filter((d: WeatherDataPoint) => {
